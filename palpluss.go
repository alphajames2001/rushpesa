...
type PalplussClient struct {
	...
	// channelIDs is the full set of channels configured via
	// PALPLUSS_CHANEL_ID (comma-separated). Which one is actually sent on
	// each deposit is resolved per-request from db (see activeChannel
	// below) so an admin switch takes effect immediately, without
...
func NewPalplussClient(cfg Config, db *DB) *PalplussClient {
...
// activeChannel resolves which channel id to use for the next request. It
// reads the admin's persisted choice (db.go's app_settings, switchable live
// from the dashboard — see admin.go's SetActivePalplussChannel) and falls
// back to the first configured channel if nothing's been set yet or if the
// persisted value is no longer in the configured set (e.g. removed from the
// env var since it was chosen).
func (p *PalplussClient) activeChannel(ctx context.Context) string {
	fallback := ""
	if len(p.channelIDs) > 0 {
		fallback = p.channelIDs[0]
	}
	active, err := p.db.GetActivePalplussChannel(ctx, fallback)
	if err != nil {
		log.Printf("palpluss: failed to read active channel, falling back to %q: %v", fallback, err)
		return fallback
	}
	...
	if active != "" {
		log.Printf("palpluss: persisted active channel %q is not in the configured channel list, falling back to %q", active, fallback)
	}
	return fallback
}
