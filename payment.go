...
// deployments don't need to change anything to keep working. db is only
// used by Palpluss, to look up the admin-switchable active channel id (see
// palpluss.go / db.go's app_settings) — Daraja doesn't need it.
func NewPaymentProvider(cfg Config, db *DB) PaymentProvider {
	switch cfg.PaymentProvider {
	case "palpluss":
		return NewPalplussClient(cfg, db)
	default:
