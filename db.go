...
//
// Backs runtime-switchable config that must survive restarts and be
// changeable from the admin dashboard without a redeploy — currently just
// the active Palpluss channel (see palpluss.go). Schema (add to your
// migrations):
//
...
const settingActivePalplussChannel = "active_palpluss_channel_id"

// GetSetting returns ErrNotFound if the key has never been set.
func (d *DB) GetSetting(ctx context.Context, key string) (string, error) {
	var value string
	const q = `select value from app_settings where key = $1`
	err := d.pool.QueryRow(ctx, q, key).Scan(&value)
	if errors.Is(err, pgx.ErrNoRows) {
		return "", ErrNotFound
	}
	return value, err
...
// GetActivePalplussChannel returns the persisted active channel id, or
// fallback (the first configured channel — see server.go's
// PalplussChannelIDs) if nothing has been set yet, e.g. right after a fresh
// deploy before an admin has ever switched channels.
func (d *DB) GetActivePalplussChannel(ctx context.Context, fallback string) (string, error) {
	value, err := d.GetSetting(ctx, settingActivePalplussChannel)
...
// SetActivePalplussChannel persists the admin's choice of active channel.
// Callers (admin.go) are responsible for validating channelID is a member
// of the configured PalplussChannelIDs before calling this.
func (d *DB) SetActivePalplussChannel(ctx context.Context, channelID string) error {
	return d.SetSetting(ctx, settingActivePalplussChannel, channelID)
}
